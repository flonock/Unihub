...
// --- EXTRACT PLANNER ---
const plannerStartIdx = pageContent.indexOf("{activeTab === 'PLANNER' && (");
const plannerEndIdx = pageContent.indexOf("{activeTab === 'OVERVIEW' && (", plannerStartIdx);
if (plannerStartIdx === -1 || plannerEndIdx === -1) {
  console.log("Could not find PLANNER block");
  process.exit(1);
}

const plannerCode = `import React from 'react';
...
fs.writeFileSync('./src/components/planner/Planner.tsx', plannerCode, 'utf8');


// --- FLASHCARD MANAGER ---
// It is at the end of the file basically.
// Instead of writing the 500 lines manually into a script, I will copy it using bash/sed directly and only write the wrapper in the script, OR just write the wrapper directly to a file and read/write the original code.
console.log("Planner extracted successfully!");
